...
class ExerciseCustomNonVideo extends StatefulWidget {
  const ExerciseCustomNonVideo({Key? key, required this.title, required this.description}) : super(key: key);

  static Widget builder(BuildContext context) {
    final args = ModalRoute.of(context)!.settings.arguments as List<dynamic>?;

    if (args == null || args.length < 4) {
      return _errorScreen('Invalid arguments supplied');
    }

    final index = args[3] as int;
    final dataPro = Provider.of<ExerciseProvider>(context, listen: false);

    if (index >= dataPro.todaysExercises.length) {
      return _errorScreen('Exercise not found');
    }
...
Widget build(BuildContext context) {
  ...
  final topHeight = size.height < 700 ? size.height * 0.55 : size.height * 0.60;
  final bottomHeight =
      _done == null ? 100.0 : 150.0; 
